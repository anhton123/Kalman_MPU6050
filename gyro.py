...
DESCRIPTION:
    ...
    process, and visualize gyroscope data
...
# Kalman Filter Code
measure_variance_x = 0.025
measure_variance_y = 0.025
measure_variance_z = 0.025
...
# Here Y is a 3x1 matrix of the collected gyroscope data
def kalman(Y):
...
def animate(i, t, x_measured, y_measured, z_measured, x_kalman, y_kalman, z_kalman):
    ...
    # processes gyroscope data to kalman filter
    Y = np.matrix([[gyro_x],[gyro_y],[gyro_z]])
    X, P = kalman(Y)    
    kal_x = X[0,0]
    kal_y = X[1,0]
    kal_z = X[2,0]
    ...
    x_measured.append(gyro_x)
    y_measured.append(gyro_y)
    z_measured.append(gyro_z)
    ...
    plt.title("Kalman Filter for Gyroscope on MPU6050")
    plt.ylabel("Acceleration (degrees/s)")
    plt.xlabel("Time (s)")
...
while True:
    ...
    gyro_y = gyro_data['y']
    gyro_z = gyro_data['z']
    print("Gyro X: {}".format(gyro_x))
    print("Gyro Y: {}".format(gyro_y))
    print("Gyro Z: {}".format(gyro_z))
    print("--------------------------")
    time.sleep(0.5)
'''